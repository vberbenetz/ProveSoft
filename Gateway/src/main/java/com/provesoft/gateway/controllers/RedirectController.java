...
public class RedirectController implements ErrorController {
    ...
    public ModelAndView redirect(HttpServletRequest request) {
        String serverFullUrl = request.getRequestURL().toString();
        String serverBaseUrl = serverFullUrl.split("/")[2].split(":")[0];
        String errorCode = request.getAttribute("javax.servlet.error.status_code").toString();

        // User not authenticated
        if ("401".equals(errorCode)) {
            return new ModelAndView("redirect:" + "http://" + serverBaseUrl + ":8080" + "/");
        }
        else {
            return new ModelAndView("redirect:" + "http://" + serverBaseUrl + ":8080" + "/ui/");
        }
